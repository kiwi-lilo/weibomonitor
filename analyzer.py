...
from keywords import (
    ...
    OFFICIAL_NAME_YELLOW, OFFICIAL_REASON_EXPLICIT,
...
def is_official(w: Weibo) -> tuple[bool, str]:
    ...
    if w.verified_type == 0:                     # 黄V：只过滤明确官方账号
        for kw in OFFICIAL_NAME_YELLOW:
            if kw in w.user:
                return True, f"黄V昵称官方特征[{kw}]"
        for kw in OFFICIAL_REASON_EXPLICIT:
            if kw in w.verified_reason:
                return True, f"黄V认证信息[{kw}]"
        return False, ""
...
GOVERNMENT_DUTY_TERMS = (
    ...
    "供电", "燃气", "民政", "法院", "检察院", "纪委", "林业", "林草",
    "野生动物保护", "野生动物", "自然保护区", "生态保护", "园林", "路政",
    "公路管理", "市政", "交通运输", "文物",
)
...
PUBLIC_OBJECT_TERMS = (
    ...
    "垃圾", "排污", "农民工", "工资", "工钱", "国道", "省道", "县道", "乡道",
    "隧道", "护栏", "铁丝网", "野生动物", "保护区", "公共设施", "摄像头", "监控",
    "路灯", "信号灯", "充电桩", "楼盘", "房屋", "消防通道", "水泵房", "高层",
)
...
PUBLIC_PROBLEM_TERMS = (
    ...
    "不作为", "推诿", "扯皮", "未处理", "不处理", "威胁", "误伤", "伤害", "干扰",
    "迁徙", "生命安全", "安全隐患", "安全风险", "风险", "被盗", "盗窃", "失窃",
    "盗抢", "失灵", "未安装", "无证", "非法", "黑飞", "占用", "堵塞", "破损",
    "老化", "坠落", "漏电", "渗漏", "开裂", "倾斜", "未成年人", "卖淫", "赌博",
    "异味", "臭味", "缺斤少两", "虚假宣传", "不退费", "不退款",
)
...
def llm_summarize(top_candidates: list, settings) -> None:
    ...
    for w in top_candidates:
        ...
        prompt = (
            "请作为政务舆情编辑，根据原帖正文和配图，撰写一篇高质量的舆情信息报送文本。"
            "符号与首句概括：文本最开头必须带有“△”符号，且紧跟其后的第一句话必须是一句简短的话，用于精准概括该舆情事件的核心。"
            "字数与排版格式：生成的文本总字数需严格控制在180至260字。必须采用纯粹的一段话形式输出，首尾贯通，绝对不可分段。"
            "核心内容与数据：必须客观说清楚事情的来龙去脉（包含事发地点、前因后果等核心要素）。若提供的原文素材中包含任何具体数据，按信息价值择要保留，不要求机械全部罗列。"
            "责任性质界定：确保舆情事件发生的责任主体严格限定在政府行政及公共服务工作范畴内，坚决禁止从党口相关工作、司法程序、官员贪腐、普通民事纠纷或诉讼案件等角度进行撰写。"
            "公文文风语态：删除情绪宣泄、夸张修辞、反问、口号、重复表述和无信息量套话,保持严肃、紧凑的公文语态，行文要高度凝练，坚决禁止使用如“一是、二是，首先、其次”等罗列型连接词。"
            "有配图时，仅结合图片中清晰可辨且与事件直接相关的文字、标识、场景或证据。"
            f"原帖内容：{source_text}"
        )
