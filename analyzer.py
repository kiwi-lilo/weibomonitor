...
def llm_summarize(top_candidates: list, settings) -> None:
    ...
    for w in top_candidates:
        ...
        prompt = (
            "请作为专业政务舆情分析员，根据我提供的素材，撰写一篇高质量的舆情信息报送文本。"
            "文本最开头必须带有“△”符号，且第一句话必须是对整条舆情的简短总括，直接写出核心对象、核心问题或变化；"
            "首句可参考“△西安物业收费‘一费制’推行五年落地遇阻。”这种‘对象或事件＋核心状态’的概括方式，"
            "不要把首句写成机械的信息清单。"
            "通常写180至260字；信息丰富时可适当延长。采用纯粹的一段话形式输出，不换行、不加标题、不分点。"
            "只使用原文和配图明确提供的信息，不新增事实、不推测动机、不扩大责任主体、不替任何一方下结论；"
            "对网民反映、爆料或指控保留必要的来源归属。"
            "围绕首句概括，自然交代原文中的事件经过、关键事实、实际影响、群众诉求或处置进展；"
            "该说的说清楚，不为凑字数补写，也不把原文没有的信息写成缺失说明。"
            "删除情绪宣泄、夸张修辞、反问、口号、重复表述和无信息量套话，使用第三人称、简洁克制的新闻写法。"
            "有配图时结合图片中清晰可辨的文字、标识、场景和证据补充摘要，看不清或无法确定的内容不要猜测。"
            f"原帖内容：{source_text}"
        )
