...
- **个人舆情摘要**：默认通过 DeepSeek `deepseek-v4-flash` 按“首句概括—具体事实—影响、处置或诉求”的编辑规范生成领导专报；有配图时可通过 `LLM_VISION_MODEL` 切换视觉模型读取图片文字和现场证据。移动端搜索遇到长微博时会自动请求详情接口补取全文，避免只把“全文”前的截断内容送入摘要。配置 `LLM_API_BASE / LLM_API_KEY / LLM_MODEL`（任何 OpenAI 兼容接口）即可覆盖默认文本模型，未配置 API Key 时跳过调用

摘要通常每条只调用模型一次，不触发二次模型重写；视觉模型请求不受支持或失败时会自动降级为文本模型。接口请求默认等待 60 秒，可通过 `LLM_SUMMARY_TIMEOUT` 调整。摘要前会过滤个人生活、普通消费和泛化攻略等与政府职责无关的帖子。
...
python main.py
pytest tests/ -v                    # 完整源码测试（含阿里云 index.py）
```

## GitHub Actions
配置 Secrets：`WEIBO_COOKIE`、`SMTP_SERVER`、`SMTP_PORT`、`EMAIL_SENDER`、`EMAIL_PASSWORD`、`EMAIL_RECEIVERS`、`BARK_URL`、`WECOM_WEBHOOK`、`REPORT_REPO_TOKEN`，以及用于摘要的 `LLM_API_KEY`。GitHub Actions 已固定使用 DeepSeek `https://api.deepseek.com` 与 `deepseek-v4-flash`；请将 `LLM_API_KEY` 的值更新为 DeepSeek API Key，旧 Gemini Key 不能复用。手动或本地运行时仍可用 `LLM_API_BASE`、`LLM_MODEL` 覆盖服务商和模型。
...
`index.py` 是无需第三方依赖的阿里云函数计算单文件版本。它会跨媒体合并同一新闻事件，排除天气、预警、灾害、事故等内容；标题或正文涉陕即可进入完整列表，正向词只用于判断是否生成摘要。采集后还会核验央媒原站直链和原文发布日期，拒绝 Google News 旧稿、搜索页和失效中转链接。Top 10 通过邮件、Bark 和企业微信发送，企业微信随后分页发送全部有效采集结果。
